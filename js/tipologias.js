...
function tpMBTI(){
  ...
  if(typeof FN!=='undefined')
    h+=tpSec('As oito funções cognitivas',Object.entries(FN).map(([k,v])=>
      '<div class="tpx"><b>'+k+' — '+v.nome+'</b><p>'+v.voce+'</p></div>').join('')
...
function tpENN(){
  ...
  if(KG){
    ...
    h+=tpSec('Os nove pontos',KG.etypes.map(e=>
      '<div class="tpx"><b>'+e.number+' · '+(typeof ENN_NOME!=='undefined'?ENN_NOME[e.number]:e.palmer_name)+' <i>('+e.palmer_name+')</i></b>'
      +'<p>Paixão: '+({anger:'ira',pride:'orgulho',deceit:'autoengano',envy:'inveja',avarice:'avareza',fear:'medo',gluttony:'gula',lust:'excesso',sloth:'preguiça de si'})[e.passion]+' · atenção presa em: '+e.preoccupations+'.</p></div>').join(''));
  }
...
function tpSOC(){
  ...
  if(KG){
    h+=tpSec('Os oito elementos, um a um',KG.im.map(e=>{
      const attr={extraverted:'extrovertido (objeto: olha o mundo)',introverted:'introvertido (campo: olha as relações entre as coisas)'}[e.attitude];
      return '<div class="tpx"><b>'+e.symbol+' — '+e.socionics_name+'</b>'
        +'<p>'+e.jung_name+' de Jung. Elemento '+attr+', da classe '
        +(e['class'].startsWith('rational')?'racional (julga)':'irracional (percebe)')
        +', de dinâmica '+(e.dynamics==='static'?'estática (estados)':'dinâmica (processos)')+'.</p></div>';}).join(''));
...
/* ============ DISC ============ */
const DISC_FATORES=[
  ['D','Dominância','como você lida com problemas e desafios: direto, competitivo, decidido; sob pressão, impaciente e impositivo.'],
  ['I','Influência','como você lida com pessoas: comunicativo, entusiasmado, persuasivo; sob pressão, disperso e dependente de aprovação.'],
  ['S','Estabilidade','como você lida com ritmo: constante, paciente, leal ao previsível; sob pressão, resistente a qualquer mudança.'],
  ['C','Conformidade','como você lida com regras e método: preciso, criterioso, exigente consigo; sob pressão, perfeccionista e paralisado.']];
...
function tpDISC(){
  ...
  return est
    ...
    +tpSec('Os quatro fatores',DISC_FATORES.map(([s,n,d])=>
      '<div class="tpx"><b>'+s+' — '+n+'</b><p>'+d+'</p></div>').join(''))
